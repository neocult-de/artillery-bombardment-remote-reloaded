...
-- Helper function to check for number (to verify before save to settings)
...
local function abr_on_mod_item_opened(event)
...
script.on_event(defines.events.on_mod_item_opened, abr_on_mod_item_opened)
...
local gui_change_handlers = {
    ...
    artillery_bombardment_x_textbox = function(event)
        local settings = global[event.player_index]
		if is_numeric(event.element.text) then
